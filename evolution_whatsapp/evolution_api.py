...
def _handle(response):
    ...
    if response.status_code >= 400:
        ...
        # 5xx — server-side errors; never expose raw details to users.
        if response.status_code >= 500:
            frappe.throw(
                _(f"Evolution server error ({response.status_code}). Check Error Log for details."),
                title=_("Evolution API Error"),
            )
        # 4xx — application errors; show Evolution's message but cap length.
        msg = None
        ...
        msg = str(msg or "Evolution API error")[:300]
        frappe.throw(_(msg), title=_("Evolution API Error"))
